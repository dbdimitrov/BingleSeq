    ...
    sidebarPanel(
      tabsetPanel(id = ns("goSideTabSet"),
                  tabPanel(value = ("getGOGenesTab"), title = "Get DE Genes",
                           ...
                           radioButtons(ns("goGetCondition"), label = "Condition",
                                        c("All Conditions" = 0 ,"A vs B" = 1, "B vs C" = 2, "A vs C" = 3)),
...
bulk_goData <- function(input, output, session, counts, de) {
  ...
  observeEvent(input$goGetButton, {
    ...
    rv$goGetGenes <- getDEgenes(de$deTable, input$goGetType, input$goGetPvalue,
                                input$goGetFC, as.numeric(input$goGetCondition))
  ...
  observeEvent(input$goTermButton, {
    ...
    rv$goTermTable <- runGOSEQ(rv$goGetGenes, counts$countTable, input$goTermGenome, input$goTermSymbol, input$goTermTest,
                               input$goTermFDR, input$goTermPvalue)


    output$goTermTable <- DT::renderDataTable(
      DT::datatable(rv$goTermTable, rownames = FALSE,
...
getDEgenes <- function(data, type, pvalue, fchange, condition){
  ...
  if(condition==0){ #to filter for all conditions
    condition=c(1,2,3)
  }
  ...
  #filter
  if(type==1){
    table <- subset(table,FDR < pvalue & abs(table[,condition]) > fchange) # absSig

  }else if(type==2){
    table <- subset(table,FDR < pvalue & table[,condition] > fchange) # upregSig

  }else if(type==3){
    table <- subset(table,FDR < pvalue & table[,condition] < -fchange) # downreg Sig
...
runGOSEQ <- function(deGenes, data, genome, symbol, testType, pCorrect, pValue){
...
histGoTerms <- function(data, gof){
  ...
  if(gof){
    ...
    levels(data$category) <- gsub("(.{20,}?)\\s", "\\1\n", levels(data$category))
  }
