    ...
    if(!is.null(x$app)){
      ...
      if(x$app == 1){
        ...
        appendTab(inputId = "mainPage",
                  tabPanel(title = "Load Data", sc_loadDataUI("loadTab"), value = "loadTab")
        )
        ...
        # Normalize Data ----
        normalizedData <- callModule(sc_norm, "normTab", filteredData)
        ...
        # Cluster Data ----
        finalData <- callModule(sc_clust, "clustTab", normalizedData)
        ...
        observe({
          ...
          if(!is.null(finalData$finalData)){
            # if + n of clusters + min of clusts
            output$goClustNoInputUI <- renderUI({

              numericInput(("goTab-goClustNoInput"),
                           label = "Select Cluster of Interest",
                           value = min(as.numeric(levels(finalData$finalData@active.ident))),
                           min = min(as.numeric(levels(finalData$finalData@active.ident))),
                           max = max(as.numeric(levels(finalData$finalData@active.ident))))
        ...
        appendTab(inputId = "mainPage",
                  tabPanel(title = "Load Data", bulk_loadDataUI("loadTab"), value = "loadTab")
        )
        ...
        observe({
          ...
          if(!is.null(counts$countTable)){
            ...
            output$"filterTab-prefilterTable" <- DT::renderDataTable(
              DT::datatable(generateSummary(counts$countTable), options = list(paging = FALSE, searching = FALSE), rownames = FALSE)
            )
          }
        })
