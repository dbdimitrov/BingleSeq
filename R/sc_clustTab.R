...
sc_clustUI <- function(id) {
  ...
  tagList(
    ...
    sidebarPanel(
      ...
      conditionalPanel(condition =  "input.elbowButton > 0",
                       ns = ns,

                       tags$hr(),

                       selectInput(ns("clusterPackage"),
                                   label = "Select Clustering Package",
                                   choices = list("Seurat" = 1,
                                                  "SC3" = 2,
                                                  "Monocle" = 3)),

        conditionalPanel(condition = "input.clusterPackage==1",
                         ns = ns,

                         numericInput(ns("dimensionsInput"), label = "Dimensions to be used",
                                      min = 5, max = 50, value = 10),

                         numericInput(ns("resolutionInput"), label = "Resolution parameter Value",
                                      min = 0, max = 3, value = 0.5),

                         selectInput(ns("clusterAlgCombo"), label = "Select Clustering method",
                                     choices =
                                       list("Louvain algorithm" = 1,
                                            "Louvain algorithm with multilevel refinement" = 2,
                                            "SLM algorithm" = 3))
        ),

        conditionalPanel(condition = "input.clusterPackage==2",
                         ns = ns,

                         numericInput(ns("sc3minCells"),
                         ...
                         conditionalPanel(condition = "!input.estKCheck",
                                          ns = ns,

                                          numericInput(ns("sc3ClustNoInput"), label = "Number of Clusters",
                                                       min = 3, value = 6)
                         )
        ),

        conditionalPanel(condition = "input.clusterPackage==3",
                         ns = ns,


                         numericInput(ns("monoLowerDetection"), label = "Lower Detection Limit",
                                      min = 0, max = 1, value = 0.1),

                         conditionalPanel(condition = "input.monoRedMethod=='tSNE' ",
                                          ns = ns,

                                          numericInput(ns("monodimensionNo"), label = "Dimensions to be used",
                                                       min = 3, value = 5)
                         ),



                         selectInput(ns("monoRedMethod"), label = "Select Reduction Method",
                                     choices = list(
                                       "tSNE" = "tSNE",
                         ...
                         conditionalPanel(condition = "!input.estMonoClust",
                                          ns = ns,

                                          numericInput(ns("monoClustNo"),
        ...
        conditionalPanel(condition =  "input.clustButton > 0",
                         ns = ns,
                         tags$hr(),

                         h4("Visualize Data"),

                         radioButtons(ns("clustplotType"), label = "Choose Plot Type",
                                      c("PCA plot" = "pca",
                                        "tSNE Plot" = "tsne",
                                        "Dimensions Heatmap" = "heatmap")),

                         conditionalPanel(condition = "input.clustplotType=='heatmap'",
                                          ns = ns,
                                          numericInput(ns("dimNoInput"), label = "Dimension Number",
  ...
  observeEvent(input$clustButton, {
    ...
    if(!is.null(clust$scaledData)){
      ...
      if(input$clusterPackage==1){
        clust$finalData <- sueratClust(clust$scaledData, input$dimensionsInput,
                                    input$resolutionInput, as.numeric(input$clusterAlgCombo))

      }else if(input$clusterPackage==2){

        print(1)
        clust$finalData <- sc3Cluster(clust$scaledData, input$estKCheck, input$sc3ClustNoInput, input$sc3minCells)

      }else{ # Monocle normalization is suggested by the authors
        # however using Seurat Normalization seemed to work fine

        clusts <- clusterMonocle(clust$scaledData, input$monoLowerDetection,
                                 input$monoRedMethod, input$monodimensionNo,input$estMonoClust, input$monoClustNo)
        clust$finalData@active.ident <- clusts

      }

      output$clustNoText <- renderText({

        sprintf("Number of estimated clusters is: %s", nlevels(clust$finalData@active.ident))
      })
  ...
  observeEvent(input$pcaButton, {
    if(!is.null(clust$finalData)){
      ...
      if(input$clustplotType != "heatmap"){
        clust$clustPlot <- DimPlot(clust$finalData, reduction = input$clustplotType, pt.size = 1.2)
        clustPlotName <- paste("figures/", input$clustplotType, ".png", sep="")
      } else{
        clust$clustPlot <- DimHeatmap(clust$finalData, dims = as.numeric(input$dimNoInput), cells = 1000,
                                   balanced = TRUE, fast = FALSE)
        clustPlotName <- paste("figures/", input$clustplotType, "DimNo", input$dimNoInput, ".png", sep="")
      }
...
sueratClust <- function(s_object, dimNo, resQuant, algorithmNo){
  ...
  s_object <- FindClusters(s_object, resolution = resQuant, algorithm = algorithmNo)
...
sc3Cluster <- function(s_object, estK, clustNo, cellsPerC){
  ...
  qclust <- scran::quickCluster(sce, min.size = cellsPerC, assay.type = "logcounts")
  print(qclust)

  ## Data Normalization
  sce <- scran::computeSumFactors(sce, sizes = 20, clusters = qclust, positive=TRUE, assay.type = "logcounts")
  # sce <- scater::normalize(sce)

  browseVignettes("SC3")
  if(estK){ # estimate No of clusters
...
#' @export
#' @return Seurat object with SC3 clustering data
clusterMonocle <- function(s_object, lowerDetection, redMethod, dimensionNo, estimateClust, clustNo){
  ...
  my_cds <- newCellDataSet(data,
                           phenoData = pd,
                           featureData = fd,
                           lowerDetectionLimit = lowerDetection, #* lowerDetection limit
                           expressionFamily = uninormal()) #* data type option (norm done with seurat)
  ...
  my_cds@reducedDimA <- t(s_object@reductions$pca@feature.loadings)


  ## 4. Dimension reduction
  my_cds <- reduceDimension(my_cds,
                            max_components = 2,
                            num_dim = dimensionNo,
                            reduction_method = redMethod,
                            scaling = TRUE,
                            pseudo_expr = 0,
                            norm_method = 'none', #Normalization from Seurat
                            verbose = TRUE)
  ...
  if(redMethod == "tSNE"){
    ...
    state <- pData(my_cds)$State
    names(state) <- colnames(s_object)

    # Return and use the state as cluster in the seurat object
    return(state)
