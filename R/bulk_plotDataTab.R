...
bulk_plotDataUI <- function(id) {
  ...
  tagList(
    ...
    sidebarPanel(
      selectInput(ns("selectPlotCombo"), label = "Select Plot Type",
                  choices = list("PCA plot" = "pca",
                                 "Scree plot" = "scree",
                                 "Barchart" = "bar",
      ...
      #VP or MA
      conditionalPanel(condition = "input.selectPlotCombo == 'volcano' ||
                       ...
                       numericInput(ns("plotPvalue"), label =("p-value threshold"),
                                    value = 0.05, min = 0.0001, max = 0.5),
                       fluidRow(column(3, verbatimTextOutput(ns("plotPvalue")))),

                       numericInput(ns("plotFC"), label =("Fold change threshold"),
      ...
      conditionalPanel(condition = "input.selectPlotCombo == 'volcano' ||
                       ...
                       selectInput(ns("selectConditionMA"), label = "Select DE Analysis Condition",
                                     choices = list("A vs B" = 1, "B vs C" = 2, "A vs C" = 3),
                                     selected = 1)
      ...
      # Heatmap
      conditionalPanel(condition = "input.selectPlotCombo == 'heat'",
                       ns = ns,

                       selectInput(ns("selectConditionTHM"), label = ("Select DE Analysis Condition"),
                                   choices = list("All Conditions" = 0, "A vs B" = 1, "B vs C" = 2, "A vs C" = 3)),

                       numericInput(ns("topGeneNo"), label =("Number of genes to display"),
                                    value = 30, min = 10),
                       fluidRow(column(3, verbatimTextOutput(ns("topGeneNo")))),

                       selectInput(ns("selectTypeHM"), label =("Genes of interest:"),
                                   choices = list("All Significant" = 1, "Upregulated" = 2,
                                                  "Downregulated" = 3, "Non-significant" = 4),
                                   selected = 1),
    ...
    mainPanel(
      ...
      conditionalPanel(condition = "input.selectPlotCombo == 'bar'",
                       ns = ns,
...
bulk_plotData <- function(input, output, session, rv) {
  ...
  observeEvent(input$figButton, {
    ...
    if(input$selectPlotCombo == "pca"){
      ...
      rv$plot <- plotPCA(rv$deTable, (rv$offset+1):(ncol(rv$deTable)))
      ...
      rv$barTable <- barTable(rv$deTable, rv$conditionNo,
                              as.numeric(input$plotPvalue), as.numeric(input$plotFC))

      output$barTable <- DT::renderDataTable(

        DT::datatable(rv$barTable, colnames = c("Contrast", "Direction", "Significant Gene No"))
      ...
      rv$plot <- plotHeatmapTop(rv$deTable,
                                rv$offset, rv$replicateNo,
      ...
      rv$heatData <- getHeatData(rv$deTable,
                                 rv$offset,
                                 rv$replicateNo,
                                 as.numeric(input$topGeneNo),
                                 as.numeric(input$selectTypeHM),
      ...
      rv$plot <- plotContrastVenn(rv$deTable, input$plotPvalue, input$plotFC)
  ...
  output$downloadTopHeat <- downloadHandler(
    filename = function() {
      paste(input$selectPlotCombo, "plot" ,device= ".png", sep="")
    },
    content = function(file) {
      device <- function(..., width, height) {
        grDevices::png(..., width = width, height = height,
                       res = 1200, units = "in", pointsize = 12)
      }
      ggsave(file, plot = rv$plot, device = device, width = 12, height = 8, limitsize = FALSE)
    }
  )
...
barTable <- function(data, conditionNo, pvalue, fchange){
  ...
  upSig.AB <- subset(countTable,FDR < pvalue & countTable[,1] > fchange)
  downSig.AB <- subset(countTable,FDR < pvalue & countTable[,1] < - fchange)
  ...
  if(conditionNo>2){
    # filter
    upSig.BC <- subset(countTable,FDR < pvalue & countTable[,2] > fchange)
    upSig.AC <- subset(countTable,FDR < pvalue & countTable[,3] > fchange)
    downSig.BC <- subset(countTable,FDR < pvalue & countTable[,2] < - fchange)
    downSig.AC <- subset(countTable,FDR < pvalue & countTable[,3] < - fchange)
  ...
  if(conditionNo==2){
    ...
    number_of_sig_genes = c(upCount.AB, downCount.AB, upCount.BC, downCount.BC, upCount.AC, downCount.AC)
  }

  df <- data.frame(comparison, direction, number_of_sig_genes) # Merge vector into df
...
plotBarChart <- function(df){
  p <- ggplot(data=df, aes(x=comparison, y=number_of_sig_genes, fill=direction))+
    geom_bar(colour="black", stat="identity", position = "dodge") +
    ylab("Number of significant genes") +
    xlab("Contrast") +
    scale_fill_discrete(name = "Direction") +
    theme_classic(base_size = 13) +
    geom_text(aes(label=number_of_sig_genes), position=position_dodge(width=0.9), vjust=2)
...
plotContrastVenn <- function(data, pvalue, fchange){
  ...
  x1_sig <- subset(countTable, FDR < pvalue & abs(countTable[,1]) > fchange)
  x2_sig <- subset(countTable, FDR < pvalue & abs(countTable[,2]) > fchange)
  x3_sig <- subset(countTable, FDR < pvalue & abs(countTable[,3]) > fchange)
...
getHeatData <- function(data, offset, replicates, geneNo, type, pvalue, fchange, conditionFC){
...
plotVP <- function(data, fcColumn, fcValue, pValue){
  ...
  x$sig_flag <- as.factor(x$FDR < pValue & abs(x[,fcColumn]) > fcValue)

  x <- na.omit(x)

  VP <- ggplot(data=x, aes(x[,fcColumn], y=-log10(x$FDR), colour = sig_flag)) + geom_point(size=1.8) +
    xlab("-log10 Adjusted p-value") +
    ylab("Log2 Fold Change") +
    theme_classic() +
    theme(legend.position="bottom",legend.title = element_blank()) +
    scale_colour_discrete(breaks=c("TRUE", "FALSE"),labels=c("Significant", "Non-significant"))

  # png("figures/VolcanoPlot.png", height=600, width=800)
...
#' @export
#' @return The MA plot
plotMA <- function(data, expColumns, fcColumn, fcValue, pValue){
  ...
  ## Generate a MA plot
  MA <- ggplot(data=x, aes(x=log10(x$mean_expression+0.001), y=x[,fcColumn], colour = x$sig_flag)) +
    geom_point(size=1.8) +
