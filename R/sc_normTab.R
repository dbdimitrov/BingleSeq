...
sc_normUI <- function(id) {
  ...
  tagList(
    ...
    sidebarPanel(
      ...
      selectInput(ns("normalizeCombo"), label = "Select Normalization method",
                  choices = list("Log Normalize" = "LogNormalize",
                                 "Centered log ratio transformation" = "CLR",
                                 "Relative counts" = "RC"),
                  selected = 1),

      numericInput(ns("scaleNumInput"), label = "Set Scale Factor for normalization",
                   min = 1000, value = 10000), # 1e6 for rc

      tags$hr(),



      selectInput(ns("varianceCombo"), label = "Variance Estimation Method",
                  choices = list("VST" = "vst", "Mean Variance Plot" = "mvp", "Dispersion" = "disp"),
                  selected = 1),

      numericInput(ns("varianceFeatInput"), label = "Set FeatureNo for Variance Estimation",
                   min = 100, value = 2000),
    ...
    mainPanel(
      #verbatimTextOutput("normalizeText", placeholder = T),
...
sc_norm <- function(input, output, session, filtData) {
  ...
  observeEvent(input$normalizeButton, {
    ...
    norm$normalizedData <- normalizeSeurat(filtData$filteredData, input$normalizeCombo, input$scaleNumInput,
                                         input$varianceCombo, input$varianceFeatInput)
...
normalizeSeurat <- function(s_object, normalizeMet, scaleF, varianceMet, nfeat){

  normalized_object <- NormalizeData(s_object, normalization.method = normalizeMet, scale.factor = scaleF)

  if(startsWith(varianceMet,"vst")){
    normalized_object <- FindVariableFeatures(normalized_object, selection.method = varianceMet, nfeatures = nfeat)
  }else{
    normalized_object <- FindVariableFeatures(normalized_object, selection.method = varianceMet)
  }
