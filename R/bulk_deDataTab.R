...
bulk_deDataUI <- function(id) {
  ...
  tagList(
    ...
    sidebarPanel(
        ...
        # Select DE package
        selectInput(ns("selectPackage"), label = "Select DE package",
                    choices = list("DESeq2" = 1, "EdgeR" = 2, "ALDEx2" = 3, "Limma" = 4),
                    selected = 1),
        ...
        numericInput(ns("conditionNo"), label = "Number of Conditions", value = 3, min = 2, max = 3),

        hr(),
        fluidRow(column(3, verbatimTextOutput(ns("conditionNo")))),

        # Input: Select replicate number ----
        numericInput(ns("replicateNo"), label = "Number of Replicates", value = 4, min = 1),

        hr(),
        fluidRow(column(3, verbatimTextOutput(ns("replicateNo")))),
...
bulk_deData <- function(input, output, session, fCounts) {
  ...
  observeEvent(input$dePackageButton, {
    ...
    if(as.numeric(input$selectPackage)==1){
      de$deTable <- deSequence(fCounts$filteredCounts, as.numeric(input$conditionNo), as.numeric(input$replicateNo))
      print(nrow(de$deTable))

    }else if(as.numeric(input$selectPackage)==2){
      de$deTable <- deEdgeR(fCounts$filteredCounts, as.numeric(input$conditionNo), as.numeric(input$replicateNo))
      print(nrow(de$deTable))

    }else if(as.numeric(input$selectPackage)==3){
      de$deTable <- deALDE(fCounts$filteredCounts, as.numeric(input$conditionNo), as.numeric(input$replicateNo))
      print(nrow(de$deTable))

    }else if(as.numeric(input$selectPackage)==4){
      de$deTable <- deLimma(fCounts$filteredCounts, as.numeric(input$conditionNo), as.numeric(input$replicateNo))
      print(nrow(de$deTable))

    }

    output$deTable <- DT::renderDataTable(
      DT::datatable(de$deTable[,1:(de$offset)], options = list(pageLength = 10))
    )
...
deEdgeR <- function(readCounts, conditionNo, replicateNo){
  ...
  if(conditionNo==2){
    ...
    samplescondition = factor(c(rep("C1",replicateNo),rep("C2",replicateNo), rep("C3",replicateNo)))
    my.contrasts <- makeContrasts(BvsA = C2-C1,CvsB = C3-C2,CvsA = C3-C1,levels=samplescondition)
...
deALDE <- function(readCounts, conditionNo, replicateNo){
  ...
  if(conditionNo==2){
    ...
    samples = c(rep("C1",replicateNo),rep("C2",replicateNo), rep("C3",replicateNo))
    samplescondition = factor(c(rep("C1",replicateNo),rep("C2",replicateNo), rep("C3",replicateNo)))
    my.contrasts <- makeContrasts(BvsA = C2-C1,CvsB = C3-C2,CvsA = C3-C1,levels=samplescondition)
    design <- model.matrix(~0+samplescondition) # without intercept
    colnames(design) <- levels(samplescondition)
  ...
  if(conditionNo==2){
    ...
    # Create object
    clr <- aldex.clr(countTable, samplescondition, mc.samples=1, denom="all", verbose=FALSE)

    # Calculate differences and Normalized counts
    resdata <- aldex.effect(clr, verbose = TRUE, include.sample.summary = TRUE)

    # perform DE with ALDEx2
    DEData <- aldex.ttest(clr, paired.test = FALSE, hist.plot = FALSE)
    ...
    resdata <- resdata[order(row.names(resdata)),c(4:(3+replicateNo*2))]
    ...
    # ALDEx2
    clr <- aldex.clr(countTable, samples, mc.samples=1, denom="all")
...
deLimma <- function(readCounts, conditionNo, replicateNo){
  ...
  tab <- topTable(ebayes.fit,coef = coef , number=dim(ebayes.fit)[1], genelist=fit$genes$NAME, adjust="fdr", sort.by="none")
